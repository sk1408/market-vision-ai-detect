...
export const fetchRealTimeQuote = async (symbol: string): Promise<StockQuote> => {
  try {
    ...
    try {
      ...
      if (mockData && mockData.data && Array.isArray(mockData.data) && mockData.data.length > 0) {
        const latestData = mockData.data[0];
        return {
          symbol: symbol,
...
export const fetchTimeSeriesData = async (
  ...
  try {
    ...
    try {
      const mockData = getStockData(symbol.replace('.NS', '').replace('.BO', ''));
      // Fix the data property access - mockData already has the data array directly
